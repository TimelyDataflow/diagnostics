...
pub fn open_sockets(
    ...
    let socket_addr = (ip_addr, port)
        .to_socket_addrs()?
        .next()
        .ok_or(ConnectError::Other("Invalid listening address".to_string()))?;
    ...
    Ok((0..source_peers)
        .map(|_| {
            let socket = listener
                .incoming()
                .next()
                .expect("Socket unexpectedly unavailable");
...
pub fn make_readers<T, E>(
    ...
    match source {
        ReplaySource::Tcp(sockets) => Ok(sockets
            .lock()
            .unwrap()
            .iter_mut()
            .enumerate()
            ...
            .map(|(_, s)| {
                s.take()
                    .expect("socket missing, check the docs for make_replayers")
            })
        ...
        ReplaySource::Files(files) => {
            let open_files = files
                ...
                .map(|(_, s)| {
                    s.take()
                        .expect("file name missing, check the docs for make_replayers")
                })
                .map(|p| File::open(&p))
